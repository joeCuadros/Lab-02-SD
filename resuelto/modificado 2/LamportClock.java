...
public class LamportClock {
    ...
    public synchronized void update(int receivedTime) {
        if (receivedTime != this.clock) {
            System.out.println("?????? Thread " + Thread.currentThread().threadId() + " tiene " + receivedTime + " y LamportClock " + this.clock + " = " + (Math.max(this.clock, receivedTime) + 1));
        }
        ...
        for (int i = 0; i < 5; i++) {
            Thread thread = new Thread(new Runnable() {
                ...
                public void run() {
                    ...
                    System.out.println("Thread " + Thread.currentThread().threadId() + " created event with Lamport time " + time);
                    try {
                    ...
                    System.out.println("Thread " + Thread.currentThread().threadId() + " received event with Lamport time " + receivedTime);
